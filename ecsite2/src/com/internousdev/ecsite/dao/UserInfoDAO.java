...
public class UserInfoDAO {
	public int createUser(String familyName, String firstName, String familyNameKana, String firstNameKana, String sex,
		...
		try {
			...
			preparedStatement.setString(1, loginId); // userIdの方がしっくりくる
	...
	public UserInfoDTO getUserInfo(String loginId, String password) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				if (resultSet.getString("status").equals("1")) { // 管理者でログイン用
	...
	// ここから管理者用
	public ArrayList<UserInfoDTO> getUserInfoAll() throws SQLException {
		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();

		ArrayList<UserInfoDTO> userInfoDTOList = new ArrayList<UserInfoDTO>();
		String sql = "select id, user_id, password, family_name, first_name, family_name_kana, first_name_kana, sex, email from user_info ORDER BY id ASC";
	...
	// 管理者権限でユーザー情報全権削除
	public int userListAllDelete() throws SQLException {

		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();

		String sql = "DELETE FROM user_info WHERE status = '0'";
